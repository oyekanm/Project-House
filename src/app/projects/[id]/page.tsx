...
export default function ProjectDetail({ params }: Props) {
  const { id } = params;
  const { data, isLoading }: { data: Application; isLoading: boolean } =
    FetchData(`/api/application?id=${id}`);
    ...
    return (
      <div className="Container md:w-[80%] mx-auto text-center grid gap-16">
        <section>
          ...
          <p className="text-[1.5rem] text-start font-medium ">
            {data?.Project.description}
          </p>
        </section>
        <section>
          <p className="text-[1.5rem] ">
            This Project Was Made Using The Following Stacks
          </p>
          <div className="flex gap-8 flex-wrap items-center justify-center w-full mx-auto mt-8">
            {data?.Project.stacks.map((skill) => {
              return (
                <span
                  key={skill.id}
                  className="text-white text-[1.2rem] font-semibold uppercase"
                >
                  {skill.name}
                </span>
              );
        ...
        <section>
          <p className="text-[2.5rem] font-bold uppercase">
            App Image Overview
          </p>
          <PhotoProvider>
                ...
                return (
                  <PhotoView key={img.id} src={img.url} >
                   <div className="flex flex-col gap-6">
                      ...
                      width={500}
                      height={700}
                      className="w-full h-[500px] object-contain animate-in hover:scale-105"
                    />
                    <span className="text-[1.6rem] leading-[2.4rem] cursor-pointer font-medium bg-gray-500 p-4 px-8 rounded-lg self-center">
                      preview
                    </span>
                   </div>
                  </PhotoView>
          ...
          {/* <div className="grid sm:grid-cols-3 gap-8 mt-8">
            {image?.map((img) => {
              return (
                <div className="flex flex-col gap-6" key={img.id}>
                  <Image
                    src={img.url}
                    alt={img.key}
                    width={500}
        ...
        {url && (
          <div>
            <a href={url}>
              <Button
                variant={"ghost"}
                className="p-12 text-[2rem] font-medium"
              >
